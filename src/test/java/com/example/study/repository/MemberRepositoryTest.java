...
class MemberRepositoryTest {
   ...
   //@Test (Test only one time / it will lead on making the table again)
   void testInsertData() {
   ...
   void search() {
      ...
      // when
      Member foundMember = factory.selectFrom(m)
   ...
   void testFetchResult() {
      ...
      //fetchOne
      Member fetch2 = factory.selectFrom(member).where(member.id.eq(3L)).fetchOne();
   ...
   void paging() {
      ...
      List<Member> result = factory.selectFrom(member)
            ...
            .offset(3) // LIMIT (3, 3) -> To make the paging
            .limit(3)
            .fetch();
      // then
   ...
   */
   ...
   void subQueryTest() {
      ...
      List<Member> result = factory.selectFrom(member)
            .where(member.age.eq(
                  // 나이가 자장 많은 사람을 조회하는 서브쿼리문이 들어가야 함.
